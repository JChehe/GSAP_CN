...
data（\*）：可存储任意数据的地方（若 vars.data 不为空，则在初始时填入）。
...
target（Object）\[只读\]：目标元素（或对象数组），即谁的属性应用补间动画。
...
\[静态\] 用于创建一个反向补间动画的静态方法。定义的是初始值，而当前值则被用作目标值。适用于进场动画，因为可以在一开始就设定了补间动画结束时的状态，然后从其它地方过渡至此。

fromTo\( target: Object, duration: Number, fromVars: Object, toVars: Object \): TweenLite

\[静态\] 用于创建一个可定义初始和结束值的补间动画的静态方法（与 to\(\) 和 from\(\) 补间动画不同，这两者均基于目标元素的当前值作为初始值或结束值）。
...
获取或设置动画的进度，取值范围为 0~1。0 代表初始位置，0.5 代表中间位置，1 代表结束位置（完成）。
